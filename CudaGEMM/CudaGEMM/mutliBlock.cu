...
//#include <cublas_v2.h>
//
//// 通过行、列和列宽计算偏移量，在行主序矩阵中，列宽是矩阵的宽度
//#define OFFSET(row, col, ld) ((row) * (ld) + (col))
//
//// 转换为float4
//#define FETCH_FLOAT4(pointer) (reinterpret_cast<float4*>(&(pointer))[0])
...
//// N: ldB
//template <
//    const int BLOCK_SIZE_M,  // 每个线程块计算的C块的高度
//    const int BLOCK_SIZE_K,  // 每个线程块加载到共享内存的A块的宽度
//    const int BLOCK_SIZE_N,  // 每个线程块计算的C块的宽度
//    const int THREAD_SIZE_Y, // 每个线程计算的C块的高度
//    const int THREAD_SIZE_X,  // 每个线程计算的C块的宽度
...
//                FETCH_FLOAT4(frag_b[(j + 1) % 2][thread_x]) = FETCH_FLOAT4(Bs[load_stage_idx][j + 1][THREAD_SIZE_X * tx + thread_x]);
//            }
//            // 计算C THREAD_SIZE_X x THREAD_SIZE_Y
//#pragma unroll
//            for (int thread_y = 0; thread_y < THREAD_SIZE_Y; ++thread_y) {
//#pragma unroll
//                for (int thread_x = 0; thread_x < THREAD_SIZE_X; ++thread_x) {
...
//            FETCH_FLOAT4(frag_b[0][thread_x]) = FETCH_FLOAT4(Bs[load_stage_idx ^ 1][0][THREAD_SIZE_X * tx + thread_x]);
//        }
//        // 计算最后一个块的mma THREAD_SIZE_X x THREAD_SIZE_Y
//#pragma unroll
//        for (int thread_y = 0; thread_y < THREAD_SIZE_Y; ++thread_y) {
//#pragma unroll
//            for (int thread_x = 0; thread_x < THREAD_SIZE_X; ++thread_x) {
